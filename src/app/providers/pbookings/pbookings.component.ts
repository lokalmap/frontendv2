...
export class PbookingsComponent implements OnInit {
  ...
  reload(){
    ...
    this.csrv.getPBookingByUID(uid).subscribe(res=>{
...
//              let cdatev = ;
            ...
            let uid = localStorage.getItem('currentUID');
            this.csrv.postPBooking({usersDetailId:uid,clientAccId:uid,cdate:Date.now(),data:eventvar.objRef,status:0}).subscribe(res =>{
...
//                this.ctxvars[varIndx].classCBlock = "card-block bg-white";
          ...
          this.csrv.deletePBookingByUID(bkID).subscribe(res =>{
            //alert(res.id);
